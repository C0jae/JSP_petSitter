...
<body>
	 	...
	 	<c:forEach var="item" items="${glist}">
			<div class="g_container">
				<div class="g_thumbnail">
					<img alt="gallery" src="/img/${item.g_fname}">
				</div>
			</div>
		</c:forEach>
	</div>
	...
	<form method="post" name="reserve" action="ps_reserve.do">
		<div class="ps_baord_main" style="display: flex; align-content: ">
			<div>
				<div><h3>${m_addr} ${member.nick} 님</h3></div>
				<div><h2>${ps_board.title}</h2></div> <br>
				<div><h4>${ps_board.nick} 펫시터님을 소개합니다.</h4></div> <br>
				...
				<div>${member.license}</div> <br>
				<div><h4>펫시터 후기 
					<c:if test="${rate.freq == null}"> 없음 </c:if> 
					<c:if test="${rate.freq != null}"> ${rate.freq}개 </c:if>
					<c:if test="${rate.rate != null}"> ${rate.rate}점 </c:if>
				</h4></div>
				<div>후기 게시판</div>
			</div>
			<div>
				...
				<div class="ps_board_sub" style="margin: 80px">
					<div style="display: flex">
						<div><h4>${ps_board.nick} 님의 위치 :</h4></div>
						<div><h4>${member.m_addr}</h4></div>
					</div>
					<div>
	...
	<script type="text/javascript">
		...
		function money() {
		    ...
		    document.getElementById("result").innerHTML = "합계금액 : " + pay + "원";
